...
export function backwardSchedule(goal, targetDate, hoursPerDay = 2, startDate = null) {
    ...
    for (let i = 0; i < availableDays.length; i++) {
        ...
        events.push({
            id: uid(),
            goalId: goal.id,
            title: `${goal.emoji || '📝'} ${goal.title} : ${milestone.title}`,
    ...
    for (let month = 0; month < monthsAvailable; month++) {
        ...
        if ((month + 1) % 3 === 0) {
            ...
            events.push({
                id: uid(),
                goalId: goal.id,
                title: `${goal.emoji || '📊'} ${goal.title} : Quarterly Deep Review`,
    ...
    for (let week = 0; week < weeksAvailable; week++) {
        ...
        for (let s = 0; s < sessionsPerWeek; s++) {
            ...
            events.push({
                id: uid(),
                goalId: goal.id,
                title: `${goal.emoji || '💪'} ${goal.title} : ${isDeloadWeek ? '(Deload) ' : ''}${workoutType}`,
                date: formatDate(sessionDate),
                startHour: 7,
                duration,
                category,
